...
describe HexaPDF::DigitalSignature::Signatures do
  ...
  describe "add" do
    ...
    it "sets the needed information on the signature dictionary" do
      ...
      assert(sig[:ByteRange].value[2] > 925 + sig[:Contents].size * 1.0 * 2 + 2)
      assert(sig[:ByteRange].value[2] < 925 + sig[:Contents].size * 1.5 * 2 + 2)
      assert_equal([0, 925, sig[:ByteRange].value[2], 2501], sig[:ByteRange].value)
    ...
    it "handles different xref section types correctly when determing the offsets" do
      ...
      assert(sig[:ByteRange].value[2] > 1036 + sig[:Contents].size * 1.0 * 2 + 2)
      assert(sig[:ByteRange].value[2] < 1036 + sig[:Contents].size * 1.5 * 2 + 2)
      assert_equal([0, 1036, sig[:ByteRange].value[2], 2483], sig[:ByteRange].value)
    end
    ...
    it "works if the signature object is the last object of the xref section" do
      ...
      assert(sig[:ByteRange].value[2] > 3143 + sig[:Contents].size * 1.0 * 2 + 2)
      assert(sig[:ByteRange].value[2] < 3143 + sig[:Contents].size * 1.5 * 2 + 2)
      assert_equal([0, 3143, sig[:ByteRange].value[2], 380], sig[:ByteRange].value)
    end
